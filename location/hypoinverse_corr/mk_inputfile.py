        ...
        for line in fr.readlines():
            ...
            if lat_0 > 0 and lon_0 >= 0:
                for channel in ['001', '002', '003']:
                    g.write(
                        '{:<5s} {:2s}  {:3s} {:3d} {:7.4f}N{:3d} {:7.4f}E{:4d}\n'.format(
                            line[3], line[2], channel, latD, latM, lonD, lonM, int(float(line[5]) * 1000)
                        )
                    )
            if lat_0 > 0 and lon_0 < 0:
                for channel in ['001', '002', '003']:
                    g.write(
                        '{:<5s} {:2s}  {:3s} {:3d} {:7.4f}N{:3d} {:7.4f}W{:4d}\n'.format(
                            line[3], line[2], channel, latD, latM, lonD, lonM, int(float(line[5]) * 1000)
                        )
                    )
            if lat_0 <= 0 and lon_0 >= 0:
                for channel in ['001', '002', '003']:
                    g.write(
                        '{:<5s} {:2s}  {:3s} {:3d} {:7.4f}S{:3d} {:7.4f}E{:4d}\n'.format(
                            line[3], line[2], channel, latD, latM, lonD, lonM, int(float(line[5]) * 1000)
                        )
                    )
            if lat_0 <= 0 and lon_0 < 0:
                for channel in ['001', '002', '003']:
                    g.write(
                        '{:<5s} {:2s}  {:3s} {:3d} {:7.4f}S{:3d} {:7.4f}W{:4d}\n'.format(
                            line[3], line[2], channel, latD, latM, lonD, lonM, int(float(line[5]) * 1000)
                        )
                    )
        ...
        for event in self.events:
            for pick in event.stationPicks:
                if pick.phase.strip() in phases:
                    self.show([
                        pick.net, pick.sta, pick.phase, event.year, event.month,
                        event.day, pick.dtime, event.lon, event.lat, event.depth
                    ])
            print("Eq: ", event.no)
    ...
    def makeHypoPhase(self):
        ...
        for event in self.events:
            ...
            if lat1 > 0 and lon1 >= 0:
                print(eventFormat1.format(
                    event.year, event.month, event.day,
                    int(hour), int(minute), int(sec1), int(sec2),
                    lat1, lat2, lat3, lon1, lon2, lon3, dep1, dep2,
                    'L', mag1, mag2
                ))
            if lat1 > 0 and lon1 < 0:
                lon1 = (-1) * lon1
                print(eventFormat2.format(
                    event.year, event.month, event.day,
                    int(hour), int(minute), int(sec1), int(sec2),
                    lat1, lat2, lat3, lon1, lon2, lon3, dep1, dep2,
                    'L', mag1, mag2
            ...
            baseTime = otime - datetime.timedelta(
                seconds=otime.second, microseconds=otime.microsecond
            )

            # Python 2: list has no .copy()
            tmpPicks = list(event.stationPicks)
                ...
                for pick in tmpPicks[::-1]:
                    if pick.sta == sta:
                        ...
                        if pick.phase == 'S':
                            ...
                            s_flag = True
                        if p_flag:
                            ptime = otime + datetime.timedelta(
                                seconds=float(p_travel_time)
                            )
                            pDelta = ptime - baseTime
                            p_label = 'P'
                            # if(int(pDelta.seconds) > 100):
                            #     print('Error')
                            pSec = "{:02d}".format(pDelta.seconds)
                            sec1 = "{:02d}".format(int(pDelta.microseconds / 10000.0))
                        if s_flag:
                            stime = otime + datetime.timedelta(
                                seconds=float(s_travel_time)
                            )
                            sDelta = stime - baseTime
                            s_lable = 'S'
                            # if(int(sDelta.seconds) > 100):
                            #     print('Error')
                            sSec = "{:02d}".format(sDelta.seconds)
                            sec2 = "{:02d}".format(int(sDelta.microseconds / 10000.0))
