...
def main():
    """Main function to generate record section plots for a single day."""
    parser = argparse.ArgumentParser(description="Generate record section plots for a given picker and day.")
    parser.add_argument("picker", help="The name of the picker (e.g., STALTA, PhaseNet, QMigrate).")
    parser.add_argument("date", help="The date to process in YYYYMMDD format.")
    args = parser.parse_args()

    picker = args.picker
    date_str = args.date

    if picker not in PICKER_MAP:
        print(f"Error: Invalid picker '{picker}'. Must be one of {list(PICKER_MAP.keys())}")
        exit(1)
    
    picker_short = PICKER_MAP[picker]
    ...
    print(f"\n--- Processing Picker: {picker} for Date: {date_str} ---")
    ...
    if date_str not in events_by_day:
        print(f"No events found for {date_str} in {picker}'s catalog. Exiting.")
        return
